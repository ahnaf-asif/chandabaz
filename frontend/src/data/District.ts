...
export const ALL_DISTRICTS: DistrictType[] = [
  {
    id: '1',
    division_id: '1',
    name: 'Comilla',
...
import { get_division_by_name } from './Division';

const DEFAULT_DISTRICT = ALL_DISTRICTS[0];

export const get_district_by_id = (id: string) => {
  const found = ALL_DISTRICTS.find((item) => item.id === id);
  return found || DEFAULT_DISTRICT;
};

export const get_district_by_name = (name: string) => {
  const found = ALL_DISTRICTS.find((item) => item.name === name);
  return found || DEFAULT_DISTRICT;
};

export const get_district_by_bn_name = (bn_name: string) => {
  const found = ALL_DISTRICTS.find((item) => item.bn_name === bn_name);
  return found || DEFAULT_DISTRICT;
};

export const get_districts_by_division_id = (division_id: string) => {
  const res = ALL_DISTRICTS.filter((item) => item.division_id === division_id);
  return res.length > 0 ? res : [DEFAULT_DISTRICT];
};

export const get_districts_by_division_name = (division_name: string) => {
  const division = get_division_by_name(division_name);
  return get_districts_by_division_id(division.id);
};
